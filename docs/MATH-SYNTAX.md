...
Shipped: elementwise broadcasting over fixed-size `Array<T,N>`, a 1-D vector-algebra library
over dynamic `Vec<f64>`, the `**` operator, and Unicode operator aliases. That is an honest
"math syntax" surface, not "Julia-parity linear algebra."
