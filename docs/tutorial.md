...
BuildLang is "The Effects Language" -- a systems language experiment with a
...
The compiler binary is produced at `compiler/target/release/buildc` (or
`compiler\target\release\buildc.exe` on Windows, relative to the repository
root).
...
The generated C includes BuildLang's embedded runtime (strings, vectors, math types) followed by your compiled code. The `main()` function in `hello.c` looks like this:
...
This is where BuildLang shines. You write one `.bld` file and compile it to any shader target.
...
BuildLang ships with a VS Code extension for syntax highlighting, code snippets, and LSP support.

### Install

```bash
code --install-extension editors/buildlang-0.1.0.vsix
```

### Features

- **Syntax highlighting** for `.bld` files -- keywords, types, attributes, shader builtins
...
- **LSP launch support** -- `buildc lsp` starts the current server loop. The
  ...
  document symbols, workspace symbols for currently opened documents,
  semantic tokens v0, completion, hover, definition, references, formatting,
  ...
  End-to-end VS Code behavior, full compiler-backed semantic token indexing,
  and global workspace-symbol indexing are still not receipt-verified.
...
| Demo | Description |
|------|-------------|
| `vignette_shader.bld` | Screen-edge darkening |
| `ssao.bld` | Screen-space ambient occlusion |
| `bloom.bld` | Two-pass bright extraction + blur |
| `depth_of_field.bld` | Disc-kernel bokeh blur |
| `chromatic_aberration.bld` | RGB channel separation |
| `film_grain.bld` | Cinematic noise overlay |
| `color_grading.bld` | Color correction LUT |
| `edge_detect.bld` | Sobel edge detection |
| `cinematic.bld` | Combined cinematic pipeline |
| `reshade_tonemap.bld` | HDR tone mapping |

Compile any of them:

```bash
buildc bloom.bld --target hlsl -o bloom.fx
buildc ssao.bld --target glsl -o ssao.glsl
```
...
baseline documented in `STATUS.md`; the historical `buildc test` sweep is not
currently a green release gate.

```bash
buildc run tests/programs/01_hello.bld
buildc run tests/programs/03_functions.bld
buildc run tests/programs/11_structs.bld
buildc run tests/programs/12_enums.bld
```

### CLI Reference

The `buildc` compiler exposes these commands:

| Command | Description |
|---------|-------------|
| `buildc run <file>` | Compile and execute immediately |
| `buildc build <file>` | Compile to C and produce a native binary |
| `buildc lex <file>` | Show the token stream |
| `buildc parse <file>` | Show the AST |
| `buildc check <file>` | Run the type checker only |
| `buildc <file> --target <t>` | Compile to a specific backend |
| `buildc lsp` | Start the language server |
| `buildc fmt <file>` | Format a `.bld` source file |
| `buildc pkg <cmd>` | Package manager (init, add, resolve, search) |
...
1. Build and install the BuildLang compiler
...
BuildLang is one language for CPU and GPU. Write once, target everywhere.
