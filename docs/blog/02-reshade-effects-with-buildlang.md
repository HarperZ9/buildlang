# Building ReShade Effects with BuildLang

*Stop writing raw HLSL for your game mods. BuildLang gives you type safety, cross-target compilation, and clean ReShade output -- from a single source file.*
...
This boilerplate is repetitive, error-prone, and obscures the actual shader math. BuildLang eliminates it.

---

## How BuildLang Maps to ReShade

BuildLang provides shader-specific attributes and builtins that compile directly to the patterns ReShade expects. Here is how the concepts map:

| BuildLang | ReShade Output |
|------------|---------------|
...
You write the math. BuildLang writes the boilerplate.
...
Let us build one from scratch in BuildLang.

### Step 1: Define Uniforms

Uniforms are parameters that ReShade exposes as sliders in its UI overlay. In BuildLang, mark them with `#[uniform]`:
...
SSAO needs pseudo-random noise to distribute sample points. Standard noise functions work in BuildLang exactly as you would expect:
...
```bash
buildc ssao.bld --target hlsl -o ssao.fx
```
...
The BuildLang `demos/` directory includes ReShade-oriented source fixtures and
...
Screen-Space Ambient Occlusion. Disc-based sampling with depth comparison and range-weighted occlusion. 4 uniforms, ~83 lines of BuildLang. The effect described in detail above.
...
### 3. Depth of Field (`depth_of_field.bld`)
...
### 4. Chromatic Aberration (`chromatic_aberration.bld`)
...
## From BuildLang to In-Game: The Workflow

Here is the complete workflow for developing a ReShade effect with BuildLang:

```
1. Write effect.bld
   |
2. buildc effect.bld --target hlsl -o effect.fx
   |
...
5. Tweak uniforms in the ReShade UI
   |
6. Edit .bld source -> Recompile -> Reload in ReShade
```
...
You can. People do. But BuildLang gives you several advantages:
...
**Readable source.** Compare BuildLang's `tex2d(uv)` to ReShade's `tex2D(ReShade::BackBuffer, uv)`. Compare `#[fragment]` to manually writing semantics. The BuildLang version is shorter and expresses intent more clearly.

**CPU execution for testing.** Use `buildc run` to execute shader math on the CPU. Validate your algorithm with print statements before you ever load it in a game.
...
for f in demos/ssao.bld demos/bloom.bld demos/depth_of_field.bld \
         demos/chromatic_aberration.bld demos/film_grain.bld; do
    ./target/release/buildc "$f" --target hlsl -o "${f%.bld}.fx"
done
```
