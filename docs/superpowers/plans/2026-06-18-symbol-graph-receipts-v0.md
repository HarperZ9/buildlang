...
**Tech Stack:** Rust, serde JSON DTOs, BTreeMap/BTreeSet deterministic ordering, SHA-256, existing `quantac corpus verify` CLI tests.

## Global Constraints
- Receipt schema must be exactly `quantalang-symbol-graph-receipt/v0`.
...
- Successful `quantac corpus verify` output must include `symbol graph receipt: ok`.
...
## Interfaces
- `LoweredMirProgram` fields: `source_digest: MirRepresentationDigest`, `input_graph_digest: MirRepresentationDigest`, `ast: quantalang::ast::Module`, `function_effect_summaries: Vec<quantalang::types::FunctionEffectSummary>`, `module: quantalang::mir::MirModule`.
...
**Interfaces:** Consumes `temp_semantic_corpus`, `write_substrate_receipt_copy`, `quantac`; produces `write_symbol_graph_receipt_copy`.
...
fn assert_corpus_verify_rejects(corpus_root: &Path, expected_stderr: &str) {
    let output = quantac().arg("corpus").arg("verify").arg("--root").arg(corpus_root)
        .output().expect("run quantac corpus verify against symbol graph fixture");
...
| `corpus_verify_rejects_symbol_graph_schema_drift` | set `schema` to `quantalang-symbol-graph-receipt/v9` | `symbol graph receipt has unsupported schema` |
...
| `corpus_verify_rejects_symbol_graph_path_escape` | set `programs[0].path` to `../outside.quanta` | `symbol graph program.path must stay within corpus root` |
...
- [ ] **Step 1: Add imports.** Use `quantalang::ast::Module` and `quantalang::types::FunctionEffectSummary`.
...
cargo test --manifest-path compiler/Cargo.toml --bin quantac memory_layout --quiet
...
- [ ] **Step 7: Add unit tests.** Test sorted/deduped source symbols, exact-match edge creation, effect symbol sorting, lexical path rejection for `../x.quanta`, `C:\x.quanta`, `C:x.quanta`, and `\x.quanta`, and symbol model drift rejection.
...
- [ ] **Step 3: Validate substrate symbol surface.** Require source `AST`, representation `MIR`, effect anchor `quantalang-check-receipt/v1`, nonempty known gaps, and canonical equality to `receipts/symbol-graph-2026-06-18.json`.
