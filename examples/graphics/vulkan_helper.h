// vulkan_helper.h - Minimal Vulkan setup for BuildLang demos
// Copyright (c) 2024-2025 Zain Dana Harper. All Rights Reserved.
//
// This handles the Vulkan boilerplate so BuildLang code focuses on
// rendering logic.
//
...
//   3. The BuildLang code stays exactly the same

#ifndef BUILD_VULKAN_HELPER_H
#define BUILD_VULKAN_HELPER_H
...
static BuildGfxContext build_gfx_global_ctx = {0, 0, 0, 0, 0};
...
static int32_t build_gfx_init(int32_t width, int32_t height, const char* title) {
    ...
    build_gfx_global_ctx.initialized = 1;
    build_gfx_global_ctx.width = width;
    build_gfx_global_ctx.height = height;
    build_gfx_global_ctx.frame_count = 0;
    build_gfx_global_ctx.should_close = 0;
...
static void build_gfx_begin_frame(void) {
    build_gfx_global_ctx.frame_count++;
    // Only print for first few frames to avoid spam
    if (build_gfx_global_ctx.frame_count <= 3) {
...
static void build_gfx_end_frame(void) {
    ...
    if (build_gfx_global_ctx.frame_count >= 3) {
        build_gfx_global_ctx.should_close = 1;
    }
}

// Check if window should close
static int32_t build_gfx_should_close(void) {
    return build_gfx_global_ctx.should_close;
}

// Cleanup
static void build_gfx_shutdown(void) {
    printf("[GFX] Shutdown complete (%d frames rendered)\n", build_gfx_global_ctx.frame_count);
    build_gfx_global_ctx.initialized = 0;
}

#endif // BUILD_VULKAN_HELPER_H
