...
- **Type system:** Hindley-Milner inference with typed algebraic effects, plus an opt-in `#[linear]` attribute that enforces **no-cloning** — a `#[linear]` struct/enum value may be moved/consumed at most once, the type-system foundation shared by quantum qubits (no-cloning), on-chain assets (no double-spend), and resource handles. Borrows do not consume; ordinary types keep copy-like reuse. See [CHANGELOG.md](CHANGELOG.md), `examples/linear/`, and `docs/QUANTUM-HOST.md`.
