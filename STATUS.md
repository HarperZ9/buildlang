...
- **Header-backed + link FFI update (2026-06-29)**: added 22 tests for the `header "..."` / `link "..."` clauses and foreign `static` support (7 parser, 4 lowering, 10 C-backend/GeneratedCode, 1 `user_link_flags`). `cargo test --quiet` from `compiler/` stays green (lib 773, bin 44, integration 260; 0 failed) and `RUSTFLAGS=-Dwarnings cargo build` is clean.
