...
- **Native FFI update (2026-06-29)**: added 26 tests for the `header "..."` / `link "..."` clauses, foreign `static` support, and `extern "C" fn` exports (9 parser, 4 lowering, 12 C-backend/GeneratedCode, 1 `user_link_flags`). `cargo test --quiet` from `compiler/` stays green (lib 777, bin 44, integration 260; 0 failed) and `RUSTFLAGS=-Dwarnings cargo build` is clean.
