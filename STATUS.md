...
- **CLI subcommands**: `lex`, `parse`, `check`, `build`, `run`, `test`, `repl`, `version`, `doctor`, `lsp`, `fmt`, `pkg`, `watch`.
...
- **Current cargo baseline (2026-06-13)**: 651 passed, 0 failed, 11 ignored across the compiler test binaries via `cargo test --manifest-path compiler/Cargo.toml --quiet`.
