...
- **Type Checker**: Hindley-Milner inference, effect tracking, unification, trait resolution, const generics, higher-kinded types. Function types now carry declared capability effects, so callbacks such as `fn() with FileSystem` propagate typed accountability through higher-order calls, including effectful closure values, anonymous immediate closure calls, awaited async block effects, selected async future branch-effect unions, caller-side callback argument evidence such as `run(load_config)`, and refreshed provenance for assigned callback aliases. **Interprocedural lifetime analysis**: lifetime parameters in function types (`FnTy`), lifetime-aware call-site borrow tracking, return lifetime validation via unification. Unit tests across multiple files.
...
- **Current CI-shaped cargo baseline (2026-06-14)**: 785 passed, 0 failed, 10 ignored, 4 filtered across the compiler test binaries via `cargo test -- --skip spirv::tests::test_triangle --skip spirv::tests::test_write` from `compiler/`.
...
- **Runtime: Async** (1216 lines, 6 tests): Work-stealing scheduler design. Parser and type-checker surfaces exist for async blocks and `.await`, including delayed capability-effect accountability on awaited futures and branch-effect unioning for selected async futures, but the async runtime is not linked into compiled programs and async execution is not end-to-end verified.
...
## Honest Line Counts
- Compiler (Rust, `compiler/src/`): 87,021 lines -- STATUS: working core (lexer, parser, types, C backend), partial other backends/tools
- Integration Tests (Rust, `compiler/tests/`): 7,041 lines -- STATUS: working
