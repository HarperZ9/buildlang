...
- **Current CI-shaped cargo baseline (2026-06-14)**: 782 passed, 0 failed, 10 ignored, 4 filtered across the compiler test binaries via `cargo test -- --skip spirv::tests::test_triangle --skip spirv::tests::test_write` from `compiler/`.
...
- **Runtime: Async** (1216 lines, 6 tests): Work-stealing scheduler design. Parser and type-checker surfaces exist for async blocks and `.await`, including delayed capability-effect accountability on awaited futures, but the async runtime is not linked into compiled programs and async execution is not end-to-end verified.
...
## Honest Line Counts
- Compiler (Rust, `compiler/src/`): 86,955 lines -- STATUS: working core (lexer, parser, types, C backend), partial other backends/tools
- Integration Tests (Rust, `compiler/tests/`): 6,908 lines -- STATUS: working
