...
- **Type Checker**: Hindley-Milner inference, effect tracking, unification, trait resolution, const generics, higher-kinded types. Function types now carry declared capability effects, so callbacks such as `fn() with FileSystem` propagate typed accountability through higher-order calls. **Interprocedural lifetime analysis**: lifetime parameters in function types (`FnTy`), lifetime-aware call-site borrow tracking, return lifetime validation via unification. Unit tests across multiple files.
...
- **Current CI-shaped cargo baseline (2026-06-14)**: 770 passed, 0 failed, 10 ignored, 4 filtered across the compiler test binaries via `cargo test -- --skip spirv::tests::test_triangle --skip spirv::tests::test_write` from `compiler/`.
...
## Honest Line Counts
- Compiler (Rust, `compiler/src/`): 86,865 lines -- STATUS: working core (lexer, parser, types, C backend), partial other backends/tools
- Integration Tests (Rust, `compiler/tests/`): 6,414 lines -- STATUS: working
