...
- **Native FFI (header-backed extern)**: extern blocks accept an optional `header "..."` clause. The C backend emits the matching `#include` (angle-bracket form for `"<sqlite3.h>"`, quoted form for `"mylib.h"`), sorted and de-duplicated for reproducible output, and skips synthesizing a prototype for header-backed functions so the header's real declaration is authoritative. This is the native, embedded integration path for any C-ABI library. Verified at the parser, lowering, and C-backend-text layers, and end-to-end through `buildc build --emit c`. Native linking against the third-party library is performed by the user's C toolchain, not the compiler test suite. This is distinct from `runtime/ffi.rs`, which remains unused by codegen.
...
- **Header-backed FFI update (2026-06-29)**: added 10 tests for the `header "..."` clause (4 parser, 2 lowering, 4 C-backend). `cargo test --quiet` from `compiler/` stays green (lib test binary: 762 passed, 0 failed) and `RUSTFLAGS=-Dwarnings cargo build` is clean.
