...
- **LSP Server** (7,738 lines, 31 tests): Provider implementations exist for completion, hover, compiler-backed diagnostics, go-to-definition, symbols, formatting, folding ranges, code actions, and rename. Wired into CLI via `quantac lsp`; the raw dispatch path now uses structural JSON-RPC parsing and has a semantic-corpus LSP dispatch receipt for lifecycle, document sync, provider requests, code actions, rename, and compiler-backed diagnostic notifications. End-to-end VS Code extension behavior is not yet receipt-verified.
...
definition, references, symbols, formatting, folding ranges, code actions,
rename, and compiler-backed diagnostics through structural JSON-RPC parsing.
