...
- **Native FFI update (2026-06-29)**: added 28 tests for the `header "..."` / `link "..."` clauses, foreign `static` support, `extern "C" fn` exports, and the `--emit header` export header (9 parser, 5 lowering, 13 C-backend/GeneratedCode, 1 `user_link_flags`). `cargo test --quiet` from `compiler/` stays green (lib 779, bin 44, integration 260; 0 failed) and `RUSTFLAGS=-Dwarnings cargo build` is clean.
