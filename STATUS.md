...
- **LSP Server** (7,087 lines, 24 tests): Provider implementations exist for completion, hover, diagnostics, go-to-definition, symbols, and code actions. Wired into CLI via `quantac lsp`, but JSON dispatch uses manual string matching and only lifecycle messages are dispatched in the server loop - cannot serve a full VS Code session yet.
...
file:line:col positions. `quantac lsp` starts the current stdio server loop, but
real client request dispatch is still lifecycle-only; completion, hover,
diagnostics, symbol, and action providers exist in the compiler tree but are not
yet reachable as working VS Code LSP features.
