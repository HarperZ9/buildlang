...
build_debug() {
    ...
    # Bootstrap compiler
    if [ ! -f "$TARGET_DIR/bootstrap/buildc" ]; then
        info "Building bootstrap compiler..."
        build_bootstrap
    fi
    
    # Compile with debug flags
    "$TARGET_DIR/bootstrap/buildc" \
        --target "$TARGET" \
        --debug \
        --output "$TARGET_DIR/debug/build" \
        "$SRC_DIR/main.bld"
    
    # Build standard library
    build_stdlib debug
    
    success "Debug build complete: $TARGET_DIR/debug/build"
}
...
build_release() {
    ...
    "$TARGET_DIR/bootstrap/buildc" \
        ...
        --opt-level 3 \
        --lto \
        --output "$TARGET_DIR/release/build" \
        "$SRC_DIR/main.bld"
    ...
    if command -v strip &>/dev/null && [ "$PLATFORM" != "macos" ]; then
        strip "$TARGET_DIR/release/build"
    fi
    
    success "Release build complete: $TARGET_DIR/release/build"
    
    # Print binary size
    local size=$(ls -lh "$TARGET_DIR/release/build" | awk '{print $5}')
...
run_tests() {
    ...
    local target="${TARGET_DIR}/debug/build"
...
install_system() {
    ...
    if [ ! -f "$TARGET_DIR/release/build" ]; then
        build_release
    fi
    ...
    install -m 755 "$TARGET_DIR/release/build" "$prefix/bin/"
    
    # Install standard library
    install -d "$prefix/lib/build"
    install -m 644 "$TARGET_DIR/release/lib/"* "$prefix/lib/build/"
    
    # Install headers/includes
    if [ -d "$SRC_DIR/include" ]; then
        install -d "$prefix/include/build"
        cp -r "$SRC_DIR/include/"* "$prefix/include/build/"
    fi
...
dist() {
    ...
    local pkg_name="build-${VERSION}-${TARGET}"
    ...
    # Copy files
    cp "$TARGET_DIR/release/build" "$pkg_dir/bin/"
