...
This tutorial walks you through building a complete command-line application in BuildLang. We'll create `minigrep`, a simplified version of the `grep` text search tool.
...
- BuildLang installed ([Getting Started](../guide/getting-started.md))
...
First, let's handle command-line arguments. Edit `src/main.bld`:
...
Add tests to verify our search functions. Create `tests/test_search.bld`:
...
MINIGREP_CASE_INSENSITIVE=1 build run -- hello poem.txt
```
...
Here's the complete `src/main.bld`:
