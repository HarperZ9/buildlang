...
Historical pipeline: `.bld` -> `buildc` (Rust compiler) -> `.c` (C99) -> `cl.exe` (MSVC) -> `.exe` -> run
...
`tests/programs/01_hello.bld` through `tests/programs/99_calibrate_lut.bld`
...
## Self-Hosted Compiler (Written in BuildLang)

9 versions of a compiler written IN BuildLang, each adding a compiler phase. 6 of 9 compile and run to completion. 3 (v7-v9) compile but have runtime bugs (infinite loops in while-loop exit logic).
...
```
BuildLang (Rust) → compiles → self_hosted_v2.bld → C
...
Raw BuildLang source `fn add(a, b) { a + b }` → lexed → parsed → C generated → compiled → runs:
```
5    (add(2,3) = 2+3)
```
...
- Compiler: `buildc 1.0.0` (Rust, 81K LOC)
