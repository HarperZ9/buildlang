...
pub struct CBackend {
    ...
    /// Function-local QuantaString temps that originated from string literals.
...
impl CBackend {
    ...
    fn value_is_quanta_string(&self, value: &MirValue, locals: &[MirLocal]) -> bool {
        matches!(
    ...
    fn printf_specifier_for_value(&self, value: &MirValue, locals: &[MirLocal]) -> &'static str {
        match self.value_mir_type(value, locals) {
            ...
            Some(MirType::Struct(name)) if name.as_ref() == "QuantaString" => "%s",
            _ => "%s",
        }
    }
    ...
    fn print_arg_to_c(&self, value: &MirValue, locals: &[MirLocal]) -> String {
        ...
        } else if self.value_is_quanta_string(value, locals) {
            match value {
    ...
    fn emit_print_call(
        ...
        if args.is_empty() {
            ...
            let mut c_format = self.quanta_format_to_c_printf(format, &args[1..], locals);
            if newline {
                c_format.push('\n');
            }
